...
function getProfilePath() {
  ...
  return envPath || path.join(__dirname, '../style/profile.json')
}

function profileMtime() {
  try {
...
// System prompt cache — rebuilt only when profile file changes on disk
let _promptCache = null
let _promptMtime = -1

function buildSystemPrompt() {
  const mtime = profileMtime()
  if (_promptCache !== null && mtime === _promptMtime) return _promptCache
...
/**
 * Summarize a block of older conversation text into 2-3 sentences.
 * Always uses gpt-4o-mini (fast + cheap) regardless of OPENAI_MODEL.
 * Returns '' on any failure so the caller can degrade gracefully.
 */
async function summarizeContext(conversationText) {
  const apiKey = (process.env.OPENAI_API_KEY || '').trim()
  ...
  try {
    const { data } = await withRetry(() => axios.post(
      'https://api.openai.com/v1/chat/completions',
      {
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'Summarize this conversation excerpt in 2-3 sentences. Preserve key facts, topics discussed, and any decisions made. Be concise.' },
...
module.exports = { generateDraftReplies, withRetry, throttledStderr, summarizeContext }
