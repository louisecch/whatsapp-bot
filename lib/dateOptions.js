...
function extractDateOptions(text) {
  ...
  if (m3) {
    ...
    if (d1 && d2 && mIdx1 >= 0 && mIdx1 <= 11 && mIdx2 >= 0 && mIdx2 <= 11) {
      ...
      return { days: uniqDates([new Date(y1, mIdx1, d1), new Date(y2, mIdx2, d2)]), monthName: monthNames[mIdx1] }
    }
  }
  ...
  if (dM) {
    ...
    if (d && mIdx >= 0 && mIdx <= 11) {
      ...
      const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
      return { days: uniqDates([new Date(y, mIdx, d)]), monthName: monthNames[mIdx] }
    }
  }
