...
function getAutoReplyTargets(ctx) {
  const envRaw =
    (ctx?.AUTO_DRAFT_TARGETS || process.env.AUTO_DRAFT_TARGETS || '')
      .split(',')
...
function shouldAutoReplyToContact(message, ctx) {
  ...
  const enabled = truthy(ctx?.AUTO_DRAFT_ENABLED || process.env.AUTO_DRAFT_ENABLED || 'true')
...
async function sendText(message, toJid, text, options = {}) {
  ...
  if (typeof message?.client?.sendMessage === 'function') {
    return await message.client.sendMessage(toJid, { text: String(text || '') })
  }
...
bot(
  ...
  async (message, match, ctx) => {
    ...
    if (!replyJid) {
      return await message.send('❌ Could not determine who to DM. Try in a private chat.')
    }
    ...
    if (store.isGenerating(replyJid)) {
      return await sendText(message, replyJid, '⏳ Already generating a draft. Please wait…')
    }
    ...
    try {
      const options = await generateDraftReplies(targetText, extraContext)
      ...
      const errMsg = String(e?.response?.data?.error?.message || e?.message || e)
      await sendText(message, replyJid, `❌ Failed to generate draft: ${errMsg}`)
    } finally {
...
bot(
  ...
  async (message, _match, ctx) => {
    ...
    try {
      const options = await generateDraftReplies(message.text.trim())
...
bot(
  ...
  async (message, match) => {
    ...
    if (idArg) {
      ...
      if (!draft) {
        return await sendText(message, replyJid, `❌ Draft \`${idArg}\` not found or expired.`)
      }
    } else {
      const pending = store.pendingForChat(jid)
      if (pending.length === 0) {
        return await sendText(message, replyJid, '❌ No pending drafts. Use `.draft` first.')
      }
      if (pending.length > 1) {
    ...
    await sendText(message, replyJid, `✅ *Draft approved!*\n\nCopy and send this reply:\n\n${reply}`)
  }
)
...
bot(
  ...
  async (message, match) => {
    ...
    if (idArg) {
      ...
      const pending = store.pendingForChat(jid)
      if (pending.length === 0) {
        return await sendText(message, replyJid, '❌ No pending drafts to cancel.')
      }
      if (pending.length > 1) {
        const ids = pending.map((d) => `\`${d.id}\``).join(', ')
        return await sendText(
